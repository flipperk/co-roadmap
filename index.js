...
async function save(){
  ...
  if(syncStatus !== 'error'){
    ...
    try{
      ...
      const data = await res.json();
      syncStatus = data.success ? 'saved' : 'error';
    }catch(e){
...
function render(){
  buildGridTemplate();
  grid.querySelectorAll(".cell,.lane-label,.corner,.yearhead,.periodhead").forEach(n=>n.remove());
  ...
  state.lanes.forEach((lane, li)=>{
    ...
    PERIODS.forEach((p,idx)=>{
      ...
      cell.className = "cell" + (idx%2? " alt":"") + (p.year==="continu"?" continu":"") + (kind==="perifeer"?" perifeer":"");
      cell.style.gridRow = \`\${rowNum}\`; cell.style.gridColumn = \`\${idx+2}\`;
      cell.dataset.lane = lane.id; cell.dataset.period = p.id;
      // drop handlers
      ...
      cell.addEventListener("drop", e=>{
        ...
        if(it){ it.lane = lane.id; it.period = p.id; save(); render(); }
      });
      // cards in this cell
      laneItems.filter(i=>i.period===p.id).forEach(it=>{
        cell.appendChild(makeCard(it, lane));
      });
...
function makeCard(it, lane){
  const c = document.createElement("div");
  c.className = "card" + (it.links&&it.links.length?" haslinks":"") + (linkSource===it.id?" linksel":"");
  ...
  c.innerHTML = \`
    ...
    \${it.thema?\`<div class="row"><span class="thema">\${esc(it.thema)}</span></div>\`:""}\`;
...
function openDrawer(id){
  ...
  const perOpts = PERIODS.map(p=>{
    ...
    return \`<option value="\${p.id}" \${p.id===it.period?"selected":""}>\${yr}\${p.label}</option>\`;
  }).join("");
  ...
  drBody.innerHTML = \`
    ...
    <div class="two">
      ...
      <div class="fld"><label>Periode</label><select id="f_period">\${perOpts}</select></div>
    </div>
    <div class="two">
...
document.getElementById("drSave").addEventListener("click", ()=>{
  ...
  it.lane = val("f_lane"); it.period = val("f_period");
...
document.getElementById("addItem").addEventListener("click", ()=>{
  const it = { id:uid(), lane:state.lanes[0].id, period:NOW_PERIOD, title:"Nieuwe activiteit", lead:"", prio:"midden", thema:"", detail:"", links:[] };
...
export default {
  async fetch(req, env) {
    ...
    if (url.pathname === '/api/roadmap/load') {
      try {
        const data = await env.CO_ROADMAP_DATA.get('co-roadmap-state');
        return j({ success: true, state: data ? JSON.parse(data) : null });
      } catch (e) {
    ...
    if (url.pathname === '/api/roadmap/save' && req.method === 'POST') {
      try {
        ...
        await env.CO_ROADMAP_DATA.put('co-roadmap-state', JSON.stringify(state));
